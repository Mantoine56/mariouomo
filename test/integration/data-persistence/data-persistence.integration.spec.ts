...
describe('Data Persistence Tests', () => {
  ...
  const ordersStore = new Map<string, any>(); // Change to 'any' to accommodate custom properties
      ...
      find: jest.fn().mockImplementation(async (options) => {
        if (options?.where?.customer_id) {
          return Array.from(ordersStore.values()).filter(
            (o) => {
              // Type assertion to handle custom properties
              const orderWithCustomProps = o as any;
              return orderWithCustomProps.customer_id === options.where.customer_id;
            }
          );
        }
  ...
  describe('Order and OrderItem Relationship', () => {
    it('should create an order with items and establish the relationship', async () => {
      // Create test order with a custom property (customer_id)
      const order = {
        id: testOrderId,
        customer_id: uuidv4(), // Not part of Order entity but used in our test
        store_id: testStoreId,
        status: 'pending',
